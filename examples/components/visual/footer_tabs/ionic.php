<?php 
	...
	for($tab_no=0;$tab_no<6;$tab_no++) {
		...
		//add actions
		if (isset($component_data['actions']) )  {
			foreach ($component_data['actions'] as $action) {
				if ($action['code']==$tab['action']) {
					$tab['action_src'] = $tab['action'];
					$tab['action'] = isset($action['preview'])?$action['preview']:$action['angular'];
				};
			}
		}
			...
			<ion-grid>
				...
				<?php foreach ($buttons as $key=>$button) if ($button['enabled']=="yes"){?>
					<ion-col  class="ion-text-center" style="width: <?php echo $width?>%;">
						<ion-button fill="clear" class="footer-button" style="  width: 100%;"  (click)="<?php echo $button['action']?>">
							<div>
							...
							<?php if ($button['use_text']=="yes") {?>
								<br>
								<span style="color: <?php echo $button['icon_color_value']?>;">
								{{translationService.translatePhrase('PAGE_<?php echo $component_data['page_id']?>_<?php echo $component_data['page_component_id']?>_BUTTON_<?php echo $key?>_TEXT')}}
								</span>
							<?php }?>
