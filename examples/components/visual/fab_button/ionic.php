		...
		for ($i=0; $i<5;$i++) {
            ...
            if (isset($component_data['actions']) && isset($component_data['left_button_action']))  {
                foreach ($component_data['actions'] as $action) {
                    if ($action['code']==$fab['action']) {
                        $fab['action_src'] = $fab['action'];
                        $fab['action'] = isset($action['angular'])?$action['angular']:$action['angular'];
                    };
                }
            }            

			$buttons[$key]['buttons'][]=$fab;
		}
	}
		...
		<?php foreach($buttons as $key=>$button) if ($button['enabled']=="yes"){?>
			<ion-fab-list side="<?php echo $button['side']?>">
				<?php foreach($button['buttons'] as $fab) if ($fab['action']!="-") {?>
					<ion-fab-button style="--background: <?php echo $fab['color_value']?>;" (click)="<?php echo $fab['action']?>"><ion-icon style="color: <?php echo $fab['icon_color_value']?>;" name="<?php echo $fab['icon']?>"></ion-icon></ion-fab-button>
				<?php }?>
			</ion-fab-list>
