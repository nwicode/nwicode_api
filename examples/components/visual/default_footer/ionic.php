...
//echo print_r($component_data,true);
...
if (isset($component_data['actions']) && (isset($component_data['left_button_action']) || isset($component_data['right_button_action'])))  {
	foreach ($component_data['actions'] as $action) {
		...
		if ($action['code']==$component_data['right_button_action']) $right_button_action=$action['angular'];
	}
}
...
if (isset($component_data) &&  is_array($component_data)) {
	foreach($component_data['colors'] as $color) {
		...
		if ($color->color_name==$text_color) $text_color_value = $color->named;
	}
}
...
if (isset($component_data['colors'])) {
	foreach($component_data['colors'] as $color) {
		...
		if ($color->color_name==$right_button_color) $right_button_color_value = $color->named;
		if ($color->color_name==$right_button_text_color) $right_button_text_color_value = $color->named;
		if ($color->color_name==$right_button_icon_color) $right_button_icon_color_value = $color->named;
	}
}
...
?>
<div id="page_footer_0315">
    <ion-footer <?php if ($no_border) echo 'class="ion-no-border"';?>>
		<ion-toolbar style="text-align: <?php echo $text_align;?>;" color="<?php echo $background_color_value;?>">
			...
			<ion-buttons slot="start">
				...
				<?php if ($left_button_icon!="" && $lbtext!="") {?> 
					<ion-icon color="<?php echo $left_button_icon_color_value?>" slot="start" name="<?php echo $left_button_icon; ?>"></ion-icon><ion-text color="<?php echo $left_button_text_color_value?>">{{translationService.translatePhrase('PAGE_<?php echo $component_data['page_id']?>_FOOTER_LB_TITLE')}}</ion-text>
			...
			<?php }?>
			<ion-title><ion-text style="font-size: <?php echo $font_size;?>px;" color="<?php echo $text_color_value?>">{{translationService.translatePhrase('PAGE_<?php echo $component_data['page_id']?>_FOOTER_TITLE')}}</ion-text></ion-title>
			...
			<?php if ($use_right_button) {?>
				...
				<?php if ($right_button_icon!="" && $rbtext!="") {?> 
					<ion-icon color="<?php echo $right_button_icon_color_value?>" slot="end" name="<?php echo $right_button_icon; ?>"></ion-icon><ion-text color="<?php echo $right_button_text_color_value?>">{{translationService.translatePhrase('PAGE_<?php echo $component_data['page_id']?>_FOOTER_RB_TITLE')}}</ion-text>
